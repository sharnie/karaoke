...
function onPlayerReady(ev) {
  ...
  if ($('.playlist-video').length !== 0) {
    currentVideo = $('.playlist-video').first();
    PlayList.highlightCurrentSong();
    ev.target.loadVideoById(currentVideo.data('id'));
  }
}

function onPlayerStateChange(event) {
  // play next video if player stopped
  if (event.data === YT.PlayerState.ENDED) {

    currentVideo = currentVideo.next();
    PlayList.highlightCurrentSong();

    player.loadVideoById(currentVideo.data('id'));
